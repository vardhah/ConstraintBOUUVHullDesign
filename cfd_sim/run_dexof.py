...
def run_dex(mesh=0.1):
 ...
 for file_name in files:
    ...
    try:
     ...
     return np.atleast_2d(np.array(default_max*2)),vel


def main_run():
    alt_mesh=[0.5,0.4, 0.3,0.25,0.2,0.15,0.1,0.08,0.06,0.05]
    ...
    print('-**>Mesh is:',mesh, 'Fd found is:',Fd_found,type(Fd_found),'Inlet vel:',vel)
    if (Fd_found>=10000):
      for i in range(len(alt_mesh)-1):
         Fd_found,vel= run_dex(alt_mesh[i+1])
         print('-**> In optionalmesh, Fd:',Fd_found,type(Fd_found),'vel:',vel)
         if (Fd_found<10000):
            mesh=alt_mesh[i+1]
            break

    drag_data=np.array([vel,mesh,Fd_found])
